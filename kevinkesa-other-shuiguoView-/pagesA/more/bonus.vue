<template>
	<view class="pay_bodys">
		...
		<view class="pay_common" v-for="(item,index) in list" :key="item.id">
			<view class="pay_top">
				<text class="pay_top_left" :style="item.num>0?'color:#41a863;':''">{{item.num}}</text>
				<text class="pay_top_right">{{item.create_time}}</text>
			</view>
			<view class="pay_bottom">
				<text class="pay_bottom_left">{{item.action_text}}</text>
				<text class="pay_bottom_right">{{item.msg}}</text>
			</view>
		</view>
